...
const SCHEMA: Record<string, string[]> = {
  ...
  // migration 040 (seller_link) added to sendmo_links: funder, origin_address_id,
  // length_in, width_in, height_in, max_shipments — and to shipments: buyer_email,
  // recipient_user_id.
  // migration 049 (seller price band, PR10) added: est_min_cents,
  // est_max_cents, est_computed_at.
  sendmo_links: ["created_at","est_computed_at","est_max_cents","est_min_cents","expires_at","funder","height_in","id","is_test","last_decline_email_at","length_in","link_type","max_price_cents","max_shipments","notes","origin_address_id","preferred_carrier","preferred_speed","recipient_address_id","sender_name","short_code","size_hint","status","updated_at","user_id","weight_hint_oz","width_in"],
  shipments: ["buyer_email","cancel_token","cancelled_at","carrier","carrier_refund_id","created_at","delivered_at","display_price_cents","easypost_refund_status","easypost_shipment_id","easypost_tracker_id","escrow_id","height_in","id","is_live","is_test","item_description","label_url","length_in","link_id","payment_method","promised_delivery_date","public_code","rate_cents","recipient_address_id","recipient_user_id","refund_status","refund_submitted_at","sender_address_id","service","status","stripe_payment_intent_id","tracking_number","updated_at","weight_oz","width_in"],
