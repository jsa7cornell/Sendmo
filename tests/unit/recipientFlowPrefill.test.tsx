...
//
// Ported from PR #93 (2026-08-20) on 2026-08-25, adapted to the slot that is
// still reachable. That PR exercised sender='self' → origin; since 2026-08-23
// nothing in the UI writes 'self' (the "use my address" chips became a saved-
// address picker, which implies nothing about who ships), so `deferToSender`
// writing 'other' → destination is the only live path. The guard under test —
// `targetTouched` in RecipientFlowContext — is the same one either way, and it
// is untested on main: recipientFlowContext.test.tsx mocks a null session
// precisely so this effect never runs.
//
...
function Harness() {
  ...
  return (
    <div>
      <div data-testid="dest-street">{data.destinationAddress.street || "empty"}</div>
      <div data-testid="dest-name">{data.destinationAddress.name || "empty"}</div>
      <button onClick={() => updateData({ sender: "other" })}>Resolve Sender</button>
      <button
        onClick={() =>
          updateData({
            destinationAddress: {
...
describe("saved-address prefill", () => {
  it("fills the slot when the user has typed nothing there", async () => {
    const user = userEvent.setup();
    renderFlow();

    // Nothing happens until `sender` resolves — an unresolved sender means we
    // do not know which slot the account holder owns, and guessing is how the
    // wrong party's address lands on a label.
    expect(screen.getByTestId("dest-street")).toHaveTextContent("empty");

    await user.click(screen.getByText("Resolve Sender"));

    await waitFor(() =>
      expect(screen.getByTestId("dest-street")).toHaveTextContent(SAVED.street1),
    );
  });

  it("does NOT overwrite an address the user already typed", async () => {
    // The half that matters. Filling an empty slot is visible and obviously
    // desirable; not clobbering a typed one is invisible when it works and
    // destructive when it breaks — a saved address landing on top of a
    // hand-typed one reads as the app losing the user's input.
    const user = userEvent.setup();
    renderFlow();

    await user.click(screen.getByText("Type Destination"));
    await user.click(screen.getByText("Resolve Sender"));
    ...
    await waitFor(() =>
      expect(screen.getByTestId("dest-name")).toHaveTextContent("Typed Name"),
    );
    expect(screen.getByTestId("dest-street")).toHaveTextContent(TYPED_STREET);
    expect(screen.getByTestId("dest-street")).not.toHaveTextContent(SAVED.street1);
  });
});
