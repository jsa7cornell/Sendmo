...
21. **ALWAYS verify the deploy after pushing to `main`.** Every push to `main` triggers a production deploy (Vercel) and CI workflows. A push is **not "done"** until those are confirmed green — a red deploy is a production failure, not a finished task. After any push to `main`:
    ...
    - CI takes **~2.5 min** (measured 2026-08-20 after the e2e parallelism change, PR #91: 2m54s total, e2e step 52s across three consecutive runs; it was ~4 min with a 140s serial e2e step before). **Re-measure before sizing anything from this number** — a timeout, a retry budget, an alert threshold. On 2026-08-19 a `timeout-minutes: 20` was derived from a stale quote of this line and would have cancelled a genuinely green 20m49s run; the figure had been written from a single measurement and never re-checked. Read the step durations from a real run instead:
      ...
      History, because the shape matters more than the number: 35–37 min before the 2026-08-18 e2e de-rot (`workers: 1` + `retries: 2` re-running 28 failing specs at a 30s timeout each); then 6–21 min, which looked like a growing suite but was **entirely** the browser-install step — the suite itself was a flat ~126s throughout. Wait for a **conclusive** result — `scripts/ci-wait.sh [<pr>]` (exit **0** green, **1** red or confirmed-no-run, **2** indeterminate — never read 2 as either) — rather than ending the session on a still-running run. A pending run is not a green run. **Never hand-roll the wait loop.** The recurring bug is `until [ "$(gh run view ...)" = "completed" ]`: any `gh` failure yields an empty string, which never equals `completed`, so it sleeps forever. On 2026-08-19 one polled for **6h23m** against a run that had finished in 3m45s. `ci-wait.sh` wraps `gh pr checks --watch` and adds the three things it lacks — a hard deadline, tolerance for the 30–90s queue race, and a check that GitHub Actions actually reported (see the conflicting-PR trap below).
...
| `auth.breadcrumb` | frontend (via ingest) | Auth state change (SIGNED_OUT/USER_UPDATED always; SIGNED_IN only on a genuine re-auth transition — auth-js fires it on every tab focus) or once-daily INITIAL_SESSION heartbeat — session-durability Phase 0 diagnosis. `session_id` is a per-browser diag id. Sends are gated to the prod origin (`sendmo-diag-send` localStorage key is the test escape hatch), so no localhost noise lands here |
