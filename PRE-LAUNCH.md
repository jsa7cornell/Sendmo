...
**Status:** `[~]` **Server-half done + live; frontend Sentry/PostHog FULLY DEFERRED (John, 2026-07-06) — not a launch blocker.** John decided against standing up Sentry + PostHog for now ("i dont think we'll do sentry and post hog. fully deferred."). The frontend code is merged and **inert** (no `VITE_SENTRY_DSN` / `VITE_POSTHOG_KEY` ⇒ no SDK init, no monitoring network calls) and stays that way — no accounts, no env vars, no action. The alerting that actually matters for a money product is the **server half, which IS live**: `_shared/alert.ts:sendAdminAlert` emails John on every money-path error path (below). Revisit the frontend vendors post-launch if/when they're wanted; until then this item is intentionally parked, not pending. Server half 2026-07-04: `_shared/alert.ts:sendAdminAlert` fired from `label.buy_error`, `label.auto_refund_failed` (both sites), `label.flex_off_session_error`, and the stripe-webhook refund-failed path. Frontend half 2026-07-06 (decided proposal [proposals/2026-07-06_sentry-posthog-frontend-monitoring_reviewed-2026-07-06_decided-2026-07-06.md](proposals/2026-07-06_sentry-posthog-frontend-monitoring_reviewed-2026-07-06_decided-2026-07-06.md)): Sentry error capture + CrashScreen boundary + route-tagged events, PostHog pageview-only — merged inert (no SDK init until the env vars exist). **The CrashScreen error boundary is the one live win** (branded crash page instead of a white screen — works with no vendor). **👤 Remaining: NONE — deferred.** The Sentry/PostHog account creation + env-var steps are parked per John's 2026-07-06 call; if revisited later the steps are: create each project → set `VITE_SENTRY_DSN` (Production+Preview) / `VITE_POSTHOG_KEY` (Production) in Vercel → confirm "Automatically expose System Environment Variables" is ON → redeploy → admin `/label-test` "Throw test error" → verify the Sentry issue shows `release=<real sha>` + `environment=production`. `SENDMO_ADMIN_EMAIL` fallback to John's Gmail is the **intended** config (John, 2026-07-04) — no secret needed; the server alert path works today regardless.
