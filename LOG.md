...
**Finding: two of the three WISHLIST "Test / CI debt" items were stale, one badly.** The hardening pass planned around "~14 e2e failures from a missing `VITE_GOOGLE_MAPS_API_KEY`, suite non-blocking" — none of that is true anymore, and the diagnosis never was: `SmartAddressInput` calls the `autocomplete`/`place-details` Edge Functions (mocked per-spec via `page.route`), not `@googlemaps/js-api-loader`. The mocked suite has been green and **merge-blocking** since the 2026-08-18 de-rot + 2026-08-19 branch ruleset. Confirmed locally this session: **96 passed / 5 skipped / 0 failed, 1.0m** (suite grew from the 77–82 baseline with PRs #94–97). The two `continue-on-error` steps that remain are deliberate: ESLint (646 pre-existing errors — recounted this session, the "~30" in the WISHLIST was also stale; cleanup is its own next phase) and the authed e2e step (couples to live Supabase uptime by design). WISHLIST corrected accordingly.
