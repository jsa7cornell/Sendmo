...
**Review fixes folded in:** the seller is NOT emailed about their own cancel (surprised-recipient copy for a click they made = noise); admin cancels say "by our team"; the subject line carries RAW text (escaping there renders entities literally in the inbox — pinned by test); a failed send DELETES the dedup row so a transient Resend outage doesn't suppress the notice forever; unexpected log-insert errors are logged, only 23505 skips silently. **Known gap, explicitly deferred (review #4):** OQ6's "mandatory buyer notification" on a seller cancel is only satisfied when the refund resolves `submitted` — a rejected EasyPost void still leaves the buyer unemailed. Pre-existing shape shared by all flows (Email A has always been submitted-gated); tracked in WISHLIST.
...
Consequences that light up: the buy response's `short_code` is the real link's; tracking's `link_short_code`/`link_type` become truthful (Admin sale rows now badge correctly — closing PR6's stated gap); PR6's webhook/cancel-label guards go from inert to load-bearing (the mechanical test proves they're present); and the new `is_seller_sale` payload flag nulls PrintAnotherLabelCTA's link so a buyer gets "Start a new shipment" instead of "print another label" into a seller listing.

**The in-session review prevented a data-loss class:** `shipments.link_id` has been `ON DELETE CASCADE` since migration 001 — the throwaway delete (the repo's FIRST `.delete()`) could have taken the PAID SHIPMENT with it on a zero-row repoint, which PostgREST reports as *no error*. Now: the repoint is `.select()`-verified, a probe confirms nothing references the throwaway, and **migration 050 flips the FK to RESTRICT** so the class is structurally impossible. Also from the review: the rebind moved BELOW the cancel-token + `buyer_email` writes (load-bearing beats grouping if the isolate dies mid-request); tracking's seller discriminator is now `buyer_email OR link_type` (closing the alerted-on window where a failed `buyer_email` write showed the seller the buyer's receipt); and the lifecycle flips are scoped **positively** (`.eq("link_type","full_label")` — the `.neq(seller_link)` form left flex protected only by the "flex is never in_use" convention, enforced nowhere). The guards test asserts the positive form.
...
**Browser-verified:**
  spec: `tests/e2e/auth-section-and-flex-otp.spec.ts` + `tests/e2e/onboarding.spec.ts` + `tests/e2e/url-step-routing.spec.ts` — full mocked suite run in the lint-cleanup worktree via `npm run test:e2e:mock`, post-rebase over #102/#104: 106 passed / 6 skipped / 0 failed; plus 754 unit tests + `tsc -b` clean
  variants-covered: contact-step email-verified auto-advance (the behavioral edit — ref-mirror moved into an effect) via the auth/onboarding specs named above; admin, tracking, links, payment surfaces via their existing specs in the same run. Edge-function type annotations are runtime-inert (Deno strips them); the three non-type edge edits are the notifications.ts `.then` fix below, links' dropped dead destructure key, and test-db-insert's catch narrowing — each behavior-equivalent on the happy path.
...
- **11 deliberate patterns suppressed WITH REASONS, not rewritten:** reset-on-open modal effects (RefundModal, AddCardModal — the latter is the BUG-A regression magnet; its disable comment says "do not restructure casually"), fetch-effect `setLoading(true)` (no query library owns this), context-module hook exports (useAuth, useRecipientFlowContext), a test-only visibility predicate export, and RecipientFlowContext's direction ref — routed through [src/contexts/navDirection.ts](src/contexts/navDirection.ts), which narrows the refs rule's data-flow to ONE disabled line so the rule stays armed for the rest of the provider (an earlier draft used a file-level disable; the review replaced it). shadcn `ui/` files are exempt from `react-refresh` (their variant exports are the library's documented surface).
...
**The 10-angle code review of this PR found one real production bug, fixed here: [_shared/notifications.ts](supabase/functions/_shared/notifications.ts) called `.catch()` on a Supabase query builder** — a thenable with NO `.catch` method (verified `undefined` at runtime on the pinned 2.97.0). The line sits on the failure-logging path: whenever an email send failed, logging the failure itself threw `TypeError`, the `status: 'failed'` notifications_log row was never written, and the `notification.email_failed` event never fired — failed sends left no audit trail beyond console.error. Pre-existing since the file was written; surfaced precisely because this PR typed the client (`any` had hidden it from every tool). Fix is the sibling `adjustments.ts` idiom: `.then(() => {}, () => {})`. **Related gate gap, tracked in WISHLIST:** nothing type-checks `supabase/functions/` (tsc covers `src/` only; deploy bundles without `deno check`), so the new `SupabaseClient` annotations are documentation until a `deno check` step exists — that step would have caught this bug statically. The review also relocated the sanctioned direction-ref read into [src/contexts/navDirection.ts](src/contexts/navDirection.ts) so the react-hooks/refs rule is fully armed again inside the 400-line provider (a file-level disable had un-guarded it), corrected two doc drifts it caught (PLAYBOOK Rule 21's ESLint row still said "always green"; RefundModal's disable comment cited the wrong WISHLIST item), and added `npm run test:e2e:mock` so worktree sessions can run the mocked suite without `.env.local`.
...
**Category:** chore | ci
**Cross-link:** [WISHLIST.md](WISHLIST.md) Test/CI-debt section; TESTING.md CI section

**Finding: two of the three WISHLIST "Test / CI debt" items were stale, one badly.** The hardening pass planned around "~14 e2e failures from a missing `VITE_GOOGLE_MAPS_API_KEY`, suite non-blocking" — none of that is true anymore, and the diagnosis never was: `SmartAddressInput` calls the `autocomplete`/`place-details` Edge Functions (mocked per-spec via `page.route`), not `@googlemaps/js-api-loader`. The mocked suite has been green and **merge-blocking** since the 2026-08-18 de-rot + 2026-08-19 branch ruleset. Confirmed locally this session: **96 passed / 5 skipped / 0 failed, 1.0m** (suite grew from the 77–82 baseline with PRs #94–97). The two `continue-on-error` steps that remain are deliberate: ESLint (see below) and the authed e2e step (couples to live Supabase uptime by design). WISHLIST corrected accordingly.

**The lint count was a hall of mirrors.** An unscoped `npx eslint .` reports 646 errors — but `eslint.config.js` only ignored `dist`, so ESLint was traversing **15 stale agent worktrees under `.claude/worktrees/`** (each a full repo copy, multiplying every finding) plus `_archive`'s `.next` build output. Widened `globalIgnores` to `.claude/`, `_archive/`, `playwright-report/`, `test-results/`, `coverage/`. Real count: **57 errors / 2 warnings across 32 files** — the WISHLIST's original "~30" was far closer to truth than the scary recount. Lesson: **before quoting a lint/test count, check what the tool is actually traversing.**

**Actions bump (the one real item):** `actions/checkout`, `setup-node`, `upload-artifact` all v4 → **v7** (current latest majors — further along than the Node-24 deprecation notice implied; verified via release APIs, not memory) in `test.yml` + `deploy-edge-functions.yml`. Release notes checked: the majors are Node-24 runtime bumps; the only behavior change (checkout v7 blocks fork-PR checkout on `pull_request_target`/`workflow_run`) doesn't apply to these workflows. Proof = the PR's own CI run, which exercises all three actions.

**Lesson (same class as 2026-08-19's stale-branch incident): WISHLIST diagnoses are snapshots, not truth — re-verify a debt item's premise against the current tree before planning work around it.** Two of three items here died on contact with the repo.
...
**Cross-link:** follows [PR #91](https://github.com/jsa7cornell/Sendmo/pull/91) | review findings on that PR fixed here | [PR #92](https://github.com/jsa7cornell/Sendmo/pull/92)

**Browser-verified:**
  n/a-category: infra
  n/a-reason: One spec's timer strategy plus config comments; no product surface changed. The suite is the verification — 728 unit tests pass — and the converted spec was mutation-tested (below) to prove its assertions still bite.

**The headline this entry almost had was wrong, which is the point of writing it.**

Vitest's breakdown looked damning: `environment 110.72s` against `tests 32.74s`, with 40 of
66 unit files touching no DOM at all. Splitting those onto the `node` environment (jsdom
opt-in via a `// @vitest-environment jsdom` docblock) took the local suite from 25.45s to
7.14s. A 3.5x win, measured, reproduced.

**It was an artifact of a saturated machine.** That laptop had been running full Playwright
suites for hours; load average later hit **84 on 10 cores**. jsdom construction is
CPU-heavy, so contention penalises it far out of proportion. On CI — 4 dedicated vCPU, no
contention — the same aggregate `environment` figure was **29.45s, not 110.72s**.

Measured properly on CI, three samples per side:

| metric | main | with the split |
|---|---|---|
| **Duration** | ~23.1s | **~22.8s** |
| environment | 29.45s | 19.22s |
| tests | 16.89s | 12.5s |
| import | 10.17s | 15.2s |

`environment` really does fall. But `import` rises by about as much — splitting 66 files
across two environments duplicates module-graph work — and **wall clock nets to nothing**.
The split was reverted: 26 files of docblock churn for no measurable gain does not clear
the bar in CLAUDE.md rule 6.

**Three ways the local measurement lied, all worth recognising again:**
1. **The baseline was measured under load the comparison was not.** Contention does not
   scale costs uniformly; it punishes whichever component is CPU-bound.
2. **A `git stash -u` on an already-committed tree saves nothing**, so a later "old config"
   run silently re-ran the NEW config. Identical code produced 44.11s and 9.89s in the same
   minute — which should have been the tell that the machine, not the config, was the
   variable.
3. **A single CI sample proves nothing here.** main's own Duration ranges 23.0s to 36.7s
   across four consecutive runs. Any "win" smaller than that spread needs repeated samples.

**What actually shipped, because it is real:**

**Fake timers in `RecipientStepAddress.test.tsx` — 9606ms -> 162ms.** Three of its four
tests assert a NEGATIVE across the component's 2s auto-advance window, each via a real
`await new Promise(r => setTimeout(r, 2500))`. That is 7.5s of the suite spent literally
sleeping. `vi.useFakeTimers()` + `vi.advanceTimersByTimeAsync(2500)` is instant AND the
stronger assertion: a real sleep lets a loaded machine fire the timer late, outside the
window, so the test passes for the wrong reason. Visible on CI as `tests` 16.89s -> 12.5s,
the one metric that moved consistently. Note RTL's `waitFor` does not detect vitest's fake
timers, so the positive case drives the clock explicitly rather than polling a frozen one.

**Mutation-tested, because converting a negative assertion to fake timers is exactly how one
goes quietly vacuous.** Breaking the flag guard (`sessionStorage.getItem(OAUTH_PENDING_KEY)
=== "1"` -> `true`) failed 2 of the 3 negatives; the third survived correctly, being guarded
by validation rather than the flag. Breaking the validation guard (`errors.length > 0` ->
`false`) failed exactly that third one. Each test catches its own guard and nothing else.

**PR #91 review findings, fixed here.** The `workers` comment had become two stacked
contradictory blocks — "No CI override" and "hardcodes nothing" sitting directly above a
hardcoded `workers: 4`, because the follow-up commit added rationale without removing the
original. Merged into one. `vite preview` gained **`--strictPort`**: without it a taken port
does not fail — verified that it binds the OTHER IP stack (`[::1]` beside an existing
`*:5173`) and still reports success, which with two invocations per job building different
bundles (mock vs real Supabase) could serve the authed step the wrong one. Also corrected
the "build costs ~5s" note (it is paid twice per job) and the `vite.config.ts` warmup
comment, which justified itself with a CI path that stopped existing when CI moved to a
prebuilt bundle.

**Open, not fixed: `progress-bar.spec.ts` is deterministically broken in isolation.** Alone
at `--workers=1 --retries=0 --repeat-each=5` it fails **10 of 15** — two of three tests fail
every time. It expects the URL to rewrite to `/flexible/package` after the origin is skipped
and gets `/onboarding/full-label/origin`. It passes in the full suite, and CI's `retries: 2`
would absorb it regardless. **Not a flake — a deterministic failure wearing a flake's
clothes.** Filed rather than fixed: it is a routing question, not a speed one.
...
**Naming the product — decided by John, 2026-08-19, from a side-by-side.** Compacting the two description cards into the handoff's toggle removed the only place the shipping link was *named* before the user commits, which is the property the 2026-08-18 work fixed ("first-class means named, weighted and visible before the user can fail"). An interim cut restored it with a third caption for the unanswered state; **John chose the handoff's tighter copy instead.** The link is now named by the option label itself and by the caption the moment that option is taken. Recorded explicitly because a later reader will find the 2026-08-18 entry and reasonably conclude this drifted — it did not, it was narrowed on purpose. `skip-toggle.spec.ts` and `onboarding.spec.ts` assert the narrowed property rather than having the old assertion deleted.

**Correction to an earlier claim in this session.** A status note asserted that the Guestimator's dimensions render as "placeholders, not values". **That was wrong** — `handleGuestimation` has always written real values into the dimension fields, and the `placeholder` attributes on those inputs are the letters L/W/H, not the handoff's 10x7x4 defaults. The claim described a green editable result panel that is specified in the handoff but has not been built yet, so there was no behavior to depart from. John chose prefilled values (the handoff's spec) for that panel when it lands; the existing behavior already matches, so nothing changed. Flagged because a decision was solicited on a premise that did not hold.

**One real departure from the handoff.** "I'm the sender" is a separate chip, not a third option inside a two-option question: it is an identity claim driving prefill, not an answer about who supplies the address.
...
**Deploy:** **DEPLOYED.** Merged via [PR #72](https://github.com/jsa7cornell/Sendmo/pull/72) (merge commit `2f2fe3f`), 2026-08-19 UTC, after CI went green on the review-fixes commit. Verified live: frontend bundle `/assets/index-CMHd4k6g.js` on sendmo.co carries the defer-destination copy, AND the "Deploy Supabase Edge Functions" job (run 32209872070) is green — the money-path functions (rates/labels/links) are the deployed versions, and the post-merge main test run is also green. **Migration 042 WAS applied to production pre-merge** (verified live before and after: `flexible → TRUE` in the per-type CHECK; seller/full-label invariants unchanged). Safe order: the DB accepts recipient-less flexible links, but nothing creates them until this merges.
...
**Code review (independent agent) found 8 findings — 1 blocker, all fixed before merge:** (1) BLOCKER: a finished/expired draft isn't offerable (`loadResumable` → null) but the provider still hydrated it via `loadPersisted` after the redirect — last shipment's addresses silently prefilling a "new" flow; entry now `clearFlow()`s whatever it won't offer (the deleted step-0 door's reset, relocated), with an e2e pin. (2) The `sendmo:oauth_pending` flag leaked when the OAuth redirect never happened (error/cancel), wrongly authorizing a later auto-advance — cleared in the error branch. (3) The "I'm the sender" option promised "we'll use your saved address" it couldn't guarantee — copy now promises conditionally. (4) `startFreshFlow` wrote localStorage only; on a failed write the legacy sessionStorage draft resurrected — it now clears both stores first. (5) Undo left steps 10/14 in completedSteps, letting defer→undo users jump forward past an empty origin on the LABEL path — undo un-completes them. (6) Stale load-bearing docstring still said null sender is "treated as 'other'" — the exact fallback this change removes. (7) Two loosened tests re-tightened (history-replace Back assertion, exact neutral heading). (8) Dead `onBack` prop removed.
...
**👤 John-steps (dashboard, not in repo) — BOTH DONE, confirmed by John 2026-08-18:**
...
**Post-merge code review of this change found one real gap, fixed in a follow-up:** the three entry points were made inert, but `/sell` itself was never gated — `SellerBuilder` only checked for a session. So a signed-in user who guessed the URL got a fully working builder and could create and share a seller link whose buyer's card is then declined. Advertising the product as "Coming soon" raises the odds of exactly that, so the route now shows the same coming-soon notice for non-admins; admins still get through so the flow stays testable. Regression test confirmed failing without the gate.
...
**Category:** ship | Onboarding | Growth
**Deploy:** **DEPLOYED.** Merged to `main` via [PR #63](https://github.com/jsa7cornell/Sendmo/pull/63) (merge commit `c3b8024`), 2026-08-18. Vercel production build green; verified live on sendmo.co against bundle `/assets/index--e7cyRku.js`: step 0's "Who's sending the package", the "I don't have their address" escape, the "Send or receive a package" door and the "Shipping from" confirm row are all present, and the old "How do you want to ship?" heading plus "Flexible Prepaid Shipping Link" are gone. **Launch gate confirmed working:** with `VITE_ENABLE_SELLER_LINK` unset, Vite inlines the flag as a compile-time `false` and the minifier eliminates all three seller doors outright — "SendMo for Sellers", "Sell an item" and "Selling something?" are absent from the shipped bundle, so they cannot be reached even by inspection. `/sell` itself still ships (that route was never gated), unchanged from 2026-07-19.
...
**Status:** root cause **not** established. Two hypotheses have been proposed and **both were falsified by evidence**. Read the falsifications before proposing a third.

**Symptom:** a real user (joneshenryanderson@gmail.com, `cus_UvipZ9yuIGyPhr`, Mastercard **debit** ••2755) cannot save a card to activate a flex link. Seven `setup_intent.setup_failed` events across 2026-08-16. Every attempt: `setup_intent_authentication_failure`, 3DS 2.2.0, `result: "failed"`, `result_reason: "rejected"`, `authentication_flow: null` — the issuer's ACS **rejects authentication outright, presenting no challenge**, ~4s after `requires_action`. The user has no action available to them; there is nothing to complete.

**Falsified hypothesis 1 — "same incident as 2026-07-21."** Wrong Stripe object entirely. The screenshot is a card-*save* screen → **SetupIntent**; the July incident was a PaymentIntent. Queries ran against `payment_intents`, `charges`, and `payment_intent.payment_failed`, matched on email + last-4, and concluded "already diagnosed." The "only 1 failure in 14 days" claim used a denominator that structurally could not contain SetupIntent failures. **Check that the Stripe object type matches the screen before pulling data.**

**Falsified hypothesis 2 — "Link autofilled a stale name; issuer rejected the name mismatch."** Superficially strong: `billing_details.name` on `pm_0U5AXmxS6gsndgF35ot5Ki7i` was `eugene golovko` (a past shipping recipient from the user's unrelated BambuLab purchase, via `card.wallet.type = "link"`) on a card the user does own, and 3DS2 does send cardholder name to the issuer. **Killed by the 22:49 UTC attempt:** the user re-entered the card manually — `billing_details.name = null`, `wallet = null`, no Link involved — and the ACS returned a **byte-identical rejection**. Removing the bad name changed nothing. The name was a real data-quality problem but not the cause of the failure.

**Account-side causes ruled out (23:01 UTC).** A brand-new account (`jayuswastaken@gmail.com`, `1d3d606f-…`, created 23:00:26 — 35s before its SetupIntent) tried the same card ••2755 and got the identical rejection on `seti_0U5D2nxS6gsndgF3bwJ1TOFy`. **Two accounts, two Stripe customers, two SetupIntents, with Link autofill and without, name present and name null → same `result_reason: "rejected"`.** Nothing account-specific, nothing profile-specific, nothing Link-specific. The failure travels with the card.

**Where that leaves it:** the cause is issuer-side on card ••2755. The one distinction still unsettled is narrow but product-relevant — whether the issuer refuses *this card entirely*, or refuses only *off-session mandates* on it. Every SendMo attempt has requested a mandate; every transaction we can see that did *not* request one succeeded:

| Transaction | Mandate requested | Result |
|---|---|---|
| Feb 23 — BambuLab $393.70 (user's Link receipt) | no | approved |
| Feb — $5 prepaid API credit (user's Link receipt) | no | approved |
| Jul 22 — SendMo PI `pi_2TvrNlxS6gsndgF31qxx7epT` | `setup_future_usage: off_session` | `issuer_declined` / `generic_decline`, no 3DS |
| Aug 16 — SendMo SI `seti_0U4vjzxS6gsndgF3IkQ3hxqk` ×7 | `usage: off_session` | 3DS `rejected`, no challenge |

**Test that would confirm or kill it:** have the user buy a label **logged out**. [payments/index.ts](supabase/functions/payments/index.ts) applies `setup_future_usage` only when a customer resolves (`...(customerForPi ? {…} : {})`), so an anonymous checkout is a plain charge with no mandate. If that succeeds, the card is fine for one-time payments and simply cannot back a flex link — a product-level answer, not a bug, and the correct UX is "use a different card," not "try again."

**Third error worth not repeating — over-read a name mismatch as fraud.** A cardholder name ≠ account name, plus two issuer refusals, looked like unauthorized card use. It wasn't. `addresses` shows the account's sender address is 231 Canyon Drive, Portola Valley — the same address as 18 other rows under John/Katherine Anderson. Household, not an attacker.

**Meta-lesson.** Three readings, three corrections, each one confidently argued from real Stripe data. The pattern: a hypothesis that *explains* the evidence was repeatedly mistaken for one that was *tested against* it. Hypothesis 2 in particular fit every observation available at the time and still died in one attempt. **On payment failures, name the observation that would falsify the theory and go get it before writing the theory down.**
...
**DEPLOYED — [#61](https://github.com/jsa7cornell/Sendmo/pull/61)** (`099fef9` → `main`, `supabase functions deploy stripe-webhook`, 2026-08-16). `setup_intent.setup_failed` handler. Mirrors `status: "failed"` to `stripe_intents` and logs warn-severity with `error_code`, `decline_code`, card brand/last4, and `wallet_type`. Billing details deliberately excluded — Link can attach third-party PII. Scale of the blind spot it closes: **7 `setup_intent.setup_failed` events delivered by Stripe, 0 recorded by us** before this landed. Unaffected by the falsifications above — it records whatever the cause turns out to be.

**#61 verified live in prod:** the 23:01:11 `stripe.setup_intent_failed` row (`seti_0U5D2nxS6gsndgF3bwJ1TOFy`, mastercard/2755, `wallet_type: null`) is the **first card-save failure SendMo has ever recorded**. It is also what surfaced the second account — i.e. the handler paid for itself inside two minutes by producing the evidence that ruled out account-side causes.

**BLOCKED — [#62](https://github.com/jsa7cornell/Sendmo/pull/62).** Held first for correctness, now for infrastructure.
- *Correctness (resolved):* the original recovery copy ("Entering the card by hand usually clears it") was written under falsified hypothesis 2 and is **false advice** — 22:49 and 23:01 both disprove it. Copy rewritten (`e2769bf`) to point at a different card and to attribute the refusal to the bank, which is true under both remaining explanations.
- *Infrastructure (open):* **Vercel deploy rate limit hit — account-wide on Hobby, 24h reset.** Previews across all projects share one daily pool (see LOG [2026-06] Vercel Hobby rate-limit note). #62's preview build failed on it, and a prod deploy would hit the same wall. **Deliberately not merged:** merging would put it on `main` while prod stays on the old bundle, making `main` read as live when it isn't. Merge + deploy together once the limit resets.
- *Content, unchanged:* recovery affordances on all four failure paths, `AddCardModal` no longer discarding its error via remount, `FlexPaymentStep`'s poll-timeout branch no longer setting an `error` it never renders.

**Verification status:** both are `tsc` + `vite build` + 671 unit tests clean, eslint at baseline (no new debt). **Neither is browser-verified.** Before merging #62, check that Add-a-card still mounts and a normal test card still saves — the `PaymentElement` options changed, and that is the regression risk on a payments surface.

**User is not rate-limited** — 1 of 5 card-adds used in the rolling 24h ([PM_ADD_LIMIT_PER_DAY](supabase/functions/payment-methods/index.ts)). Note the two surfaces differ on retry cost: FlexPaymentStep hardcodes `retry_n=0` so retries reuse one SetupIntent, while AddCardModal seeds a fresh nonce per open and burns quota each time.
...
**Deploy:** PR #51 merged (squash `6688cf0`) 2026-07-09; `supabase functions deploy stripe-webhook` → v63 ACTIVE (first attempt 409'd against CI's concurrent merge-deploy of the same code; retry green). Migration 039 + backfill were already applied to prod pre-merge (see below).
...
**Deploy (Rule 21) — ✅ DEPLOYED 2026-07-07 00:15 UTC.** Merged via **PR [#45](https://github.com/jsa7cornell/Sendmo/pull/45)** (squash, commit `bb0226d`; John authorized). The "Deploy Supabase Edge Functions" job [run 28832254491](https://github.com/jsa7cornell/Sendmo/actions/runs/28832254491) went **green in 1m6s**: `_shared/` changed → redeployed **ALL** functions atomically, each printing `Deployed Functions on project ***: <name>` with **zero errors/warnings, no `522`, no `failed to create the graph`**. Money-path functions (`labels`, `payments`, `cancel-label`, `payment-methods`, `stripe-webhook`, `refunds`, …) all bundled their `jsr:@supabase/supabase-js` dep (script sizes ~835–936 kB confirm supabase-js resolved+bundled). **This is the end-to-end proof the esm.sh 522 deploy-failure class is eliminated** and `jsr:`/`Deno.serve` deploy cleanly. Concurrent PR #44 collision (receipt last4 + CORS) resolved pre-merge: LOG union, `tracking/index.ts` auto-merged. Rollback if ever needed: `git revert bb0226d` → redeploys pre-migration code in ~2 min.
