...
**Leading hypothesis 3 (UNCONFIRMED) — the issuer refuses off-session mandates on this card.** Every SendMo attempt has requested one; every transaction we can see that did *not* request one succeeded:
...
**HELD — [#62](https://github.com/jsa7cornell/Sendmo/pull/62)**, deliberately not merged despite being green. Its recovery copy ("Banks often refuse when saved autofill details don't match the card. Entering the card by hand usually clears it.") was written under falsified hypothesis 2, and the 22:49 attempt proves it is **false advice** — the user did exactly that and failed identically. Shipping it would send users down a known-dead path. The structural work in it is still sound and worth salvaging: recovery affordances on all four failure paths, `AddCardModal` no longer discarding its error via remount, and `FlexPaymentStep`'s poll-timeout branch no longer setting an `error` it never renders. **Needs: copy rewritten to match whatever hypothesis 3's test establishes.**
