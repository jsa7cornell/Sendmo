...
export default function SellerBuilder() {
    ...
    if (bandFetchRef.current?.key !== key) {
      bandFetchRef.current = {
        ...
        promise: fetchSellerBandQuote(
          ...
          // Quote only the carriers the seller can drop off at, so the
          // estimate matches what buyers of this link will actually see.
          carrierConstraint,
        )
          .then((res) => res.band)
  ...
  }, [step, parcel, origin, session?.access_token, carrierConstraint]);
  ...
  async function handleEditConfirm() {
    ...
    try {
      ...
      // closeSellerLink 409s on any non-active link — in practice a
      // single-use link that sold while the seller sat on this screen.
      const msg = e instanceof Error ? e.message : "";
      setReopenError(
        /Cannot close/i.test(msg)
          ? "This link has already sold, so it can't be edited. Create a new listing instead."
          : msg || "Couldn't turn off the old link",
      );
    } finally {
      setReopening(false);
    }
