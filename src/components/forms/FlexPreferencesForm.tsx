...
export default function FlexPreferencesForm({ value, onChange }: Props) {
  ...
  const selected = SPEEDS.find((s) => s.id === value.speed_preference) ?? SPEEDS[1];
  const SelectedIcon = selected.icon;

  function update(partial: Partial<FlexPreferencesValue>) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-muted rounded-2xl p-1.5 flex gap-1">
        {SPEEDS.map((s) => {
          const SIcon = s.icon;
          const isActive = value.speed_preference === s.id;
          return (
            <button
              key={s.id}
              type="button"
              onClick={() => update({ speed_preference: s.id })}
              className={`
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {CARRIERS.map((c) => (
                      <button
                        key={c.id}
                        type="button"
                        onClick={() => update({ preferred_carrier: c.id })}
                        className={`
                          px-3 py-1.5 rounded-lg text-sm border transition-all
                          ${value.preferred_carrier === c.id
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {PRICE_CAPS.map((cap) => (
                      <button
                        key={cap.value}
                        type="button"
                        onClick={() => update({ price_cap: cap.value })}
                        className={`
                          text-left rounded-xl border p-3 transition-all
                          ${value.price_cap === cap.value
                            ? "border-primary bg-primary/10"
                            : "border-border bg-card hover:border-muted-foreground/40"
                          }
                        `}
                      >
                        <span className={`text-base font-bold ${value.price_cap === cap.value ? "text-primary" : "text-foreground"}`}>
                          {cap.label}
                        </span>
