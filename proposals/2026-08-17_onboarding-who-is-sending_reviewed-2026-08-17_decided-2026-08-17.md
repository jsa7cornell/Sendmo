...
**OQ1 — door wording. Took the reviewer's recommendation: job-led "Sell an item"** with "The buyer pays for shipping" underneath. Cheap to reverse — it's one string.
...
Approved to build with the review's blockers folded in. OQ1–OQ4 took the reviewer's recommended defaults; **OQ1 (door wording) and OQ3 (positioning) were not separately ruled on by John** and are flagged as cheap to reverse — OQ1 is one string, OQ3 is the hero subcopy plus a SPEC paragraph. If either should go the other way, say so and it's a small follow-up, not a rework.
