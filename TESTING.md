...
- **Setup:** `.env.local` needs `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` (both publishable/public) so the dev server boots — the mocked suite needs nothing more. **No `.env.local` (fresh worktree)?** Run `npm run test:e2e:mock` — it supplies the same mock values CI's mocked step uses. **First check nothing is listening on 5173** (`lsof -ti:5173 -sTCP:LISTEN`): `reuseExistingServer` is on locally, so an already-running dev server — e.g. the main checkout's, with real env — gets reused and you'd silently test the wrong tree. Browser-verifying a **Stripe surface** (the payment step, `/label-test`) additionally needs a real `VITE_STRIPE_PUBLISHABLE_KEY_TEST` (`pk_test_…`, also publishable).
...
`.github/workflows/test.yml` ("Provide Tests") runs on every push and PR to `main`: ESLint, `tsc -b`, unit tests, the mocked e2e suite, and a scoped authed-e2e step (real Supabase — the `/links/new` phone-gate spec). **ESLint, `tsc`, unit, and the mocked e2e suite are all blocking** — the e2e step became blocking on 2026-08-18 once the host-mismatch bug was fixed and it ran deterministically green, since 2026-08-19 a branch ruleset makes the job a required check on `main`, and lint became blocking on 2026-08-23 when the 57-error backlog was cleared to 0 (keep it there; suppressions need a written reason — see LOG that date). The **authed** e2e step stays non-blocking (`continue-on-error`) on purpose: it hits live Supabase, so gating merges on it would couple every merge to a third party's uptime. A green run therefore certifies the mocked suite but not the authed step — check the run's annotations for that one.
